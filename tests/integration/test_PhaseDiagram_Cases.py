...
def test_simple_rheology():
    '''
    A simple case with no composition, a composite rheology 
    from input (case.json)
    Asserts:
        a. the prm file generated
    '''
    # test 0: change compositions in the CDPT model
    source_case_dir = os.path.join(source_dir, "test_simple_rheology")
    json_path = os.path.join(source_case_dir, 'case.json')
    output_dir = os.path.join(test_dir,'test_PhaseDiagram_simple_rheology')
...
def test_simple():
    ...
    '''
    # test 0: change compositions in the CDPT model
    source_case_dir = os.path.join(source_dir, "test_simple")
    json_path = os.path.join(source_case_dir, 'case.json')
    output_dir = os.path.join(test_dir,'test_PhaseDiagram_simple')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
    create_case_with_json(json_path, CASE, CASE_OPT)
    assert(os.path.isdir(output_dir))  # check case generation
...
def test_bd_lsolver_3D():
    ...
    prm_path = os.path.join(output_dir, 'case.prm')
    assert(filecmp.cmp(prm_path, prm_std_path))
    # test 2: tangential bc on top and bottom, no slip on both sides, for 3d, chunck geometry
    source_case_dir = os.path.join(source_dir, "test_bd_lsolver_ns_3D")
    json_path = os.path.join(source_case_dir, 'case.json')
    output_dir = os.path.join(test_dir,'test_PhaseDiagram_bd_lsolver_ns_3D')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
    create_case_with_json(json_path, CASE, CASE_OPT)
    assert(os.path.isdir(output_dir))  # check case generation
