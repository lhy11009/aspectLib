...
def test_chunk0():
    ...
    '''
    source_case_dir = os.path.join(source_dir, "test_chunk0")
...
def test_plate_width():
    '''
    test for the reference case after iteration gamma
    '''
    source_case_dir = os.path.join(source_dir, "test_plate_width")
    json_path = os.path.join(source_case_dir, 'case0.json')
    output_dir = os.path.join(test_dir,'test_plate_width')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
    create_case_with_json(json_path, CASE, CASE_OPT)
    assert(os.path.isdir(output_dir))  # check case generation
