...
Examples of usage:
  ...
  - default usage: plot case running results
        ...
        Lib_LatentHeatBK0_PlotCase plot_case -i /home/lochy/ASPECT_PROJECT/TwoDSubduction/non_linear32/eba1_MRf12_iter20
...
from shilofue.LatentHeatBK0.PlotVisit import VISIT_OPTIONS, PREPARE_RESULT_OPTIONS
...
\n\
        Lib_LatentHeatBK0_PlotCase plot_case -i  ~/ASPECT_PROJECT/TwoDSubduction/EBA_CDPT1/eba_cdpt_SA80.0_OA40.0\
 -t 0.0 -t1 0.5e6\n\
\n\
  - plot cases in a directory (loop), same options as before:\n\
        Lib_LatentHeatBK0_PlotCase  plot_case_in_dir -i ~/ASPECT_PROJECT/TwoDSubduction/EBA_CDPT\n\
\n\
  - prepare result of a single step by combining figures:\n\
        Lib_LatentHeatBK0_PlotCase prepare_result_step -i ~/ASPECT_PROJECT/TwoDSubduction/test_peierls1/peierls\n\
\n\
  - plot and then prepare result of a single step by combining figures:\n\
        Lib_LatentHeatBK0_PlotCase plot_prepare_result_step -i ~/ASPECT_PROJECT/TwoDSubduction/test_peierls1/peierls\n\
\n\
  - generate animation: \n\
        Lib_LatentHeatBK0_PlotCase animate_case -i ~/ASPECT_PROJECT/TwoDSubduction/test_peierls1/peierls\n\
\n\
  - generate animation for cases in a directory: \n\
        Lib_LatentHeatBK0_PlotCase animate_case_in_dir -i `pwd`\n\
\n\
        ")
...
def PlotCaseRun(case_path, **kwargs):
    ...
    Returns:
        -
    '''
    print("PlotCaseRun in LatentHeatBK0: operating")
    # get case parameters
    ...
    if not os.path.isdir(odir):
        os.mkdir(odir)
    py_script = 'default.py'
    ofile = os.path.join(odir, py_script)
    visit_script = os.path.join(ASPECT_LAB_DIR, 'visit_scripts', py_script)
...
def main():
    ...
    default = os.path.join(ASPECT_LAB_DIR, "files", "LatentHeatBK", "figure_step_template.json")
